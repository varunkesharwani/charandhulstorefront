...
const ImageGallery = ({ images }: ImageGalleryProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      <div
        className="flex-1 relative aspect-square md:aspect-[29/34] w-full h-[391px] md:h-[510px]"
        {...handlers} // Apply swipe handlers here
      >
        <Container className="relative w-full h-full overflow-hidden bg-ui-bg-subtle">
          <Image
            src={images[currentIndex].url}
            alt={`Product image ${currentIndex + 1}`}
            layout="fill"
            className="absolute inset-0 rounded-md object-contain" // Changed to object-contain
            sizes="(max-width: 576px) 391px, (min-width: 576px) 510px"
          />
        </Container>
      </div>
